...
require_once __DIR__ . "/Gestion_projet.php";





$select_projets = $pdo->query("
    SELECT
        p.id,
        p.titre,
        p.priorite,
        p.statut,
        c.nom        AS client,
        GROUP_CONCAT(u.prenom SEPARATOR ', ') AS assigne
    FROM projets p
    JOIN clients      c ON p.client_id = c.id
    JOIN projets_collaborateurs pc ON p.id = pc.projet_id
    JOIN utilisateurs u ON pc.utilisateur_id = u.id
    GROUP BY p.id, p.titre, p.priorite, p.statut, c.nom
");
$projets = $select_projets->fetchAll();
...
<section class="Modularecell">
    ...
    <form class="form_question" id="projectForm" method="POST">
...
<tbody>
    ...
    <?php foreach ($projets as $pr): ?>
        <tr>
    <td><?= htmlspecialchars($pr["titre"], ENT_QUOTES, 'UTF-8') ?></td>
    <td><?= htmlspecialchars($pr["client"], ENT_QUOTES, 'UTF-8') ?></td>
    <td><?= htmlspecialchars($pr["priorite"], ENT_QUOTES, 'UTF-8') ?></td>
    <td><?= htmlspecialchars($pr["statut"], ENT_QUOTES, 'UTF-8') ?></td>
    <td><?= htmlspecialchars($pr["assigne"], ENT_QUOTES, 'UTF-8') ?></td>
    <td><a href="Projet0001.php?id=<?= $pr['id'] ?>">Edit</a></td>
      </tr>
    <?php endforeach; ?>
</tbody>
</table>
