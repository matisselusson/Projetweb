...
$stmt = $pdo->prepare("
    ...
    JOIN utilisateurs u ON t.assigne_id = u.id
    WHERE t.id = :id
");
$stmt->execute([':id' => $id]);
$tk = $stmt->fetch();
...
<section class="mySectionticket">
  ...
  <div class="cell" id="ticket_title">
    ...
    <div class="number-cell"><?= htmlspecialchars($tk["titre"],       ENT_QUOTES, 'UTF-8') ?></div>
  </div>

  <div class="cell" id="id_ticket">
    <div class="title-cell">Description</div>
    <div class="number-cell"><?= htmlspecialchars($tk["description"], ENT_QUOTES, 'UTF-8') ?></div>
  </div>

  <div class="cell" id="ticket_priority">
    <div class="title-cell">Priorité</div>
    <div class="number-cell"><?= htmlspecialchars($tk["priorite"],    ENT_QUOTES, 'UTF-8') ?></div>
  </div>

  <div class="cell" id="ticket_status">
    <div class="title-cell">Statut</div>
    <div class="number-cell"><?= htmlspecialchars($tk["statut"],      ENT_QUOTES, 'UTF-8') ?></div>
  </div>

  <div class="cell" id="ticket_type">
    <div class="title-cell">Type</div>
    <div class="number-cell"><?= htmlspecialchars($tk["type"],        ENT_QUOTES, 'UTF-8') ?></div>
  </div>

  <div class="cell" id="ticket_projet">
    <div class="title-cell">Projet</div>
    <div class="number-cell"><?= htmlspecialchars($tk["projet"],      ENT_QUOTES, 'UTF-8') ?></div>
  </div>

  <div class="cell" id="ticket_assignee">
    <div class="title-cell">Assigné à</div>
    <div class="number-cell"><?= htmlspecialchars($tk["assigne"],     ENT_QUOTES, 'UTF-8') ?></div>
  </div>
